...
function StatCard({
  ...
  return (
    <article className="bg-white border border-line rounded-[10px] p-[18px] max-sm:p-[14px] hover:shadow-sm transition-shadow">
      <span className="text-[11px] text-muted font-semibold block mb-[10px]">{label}</span>
      <b className={`block font-serif text-[30px] max-sm:text-[24px] font-medium leading-none mb-[6px] ${accent}`}>
        {value}
      </b>
...
function statusBadgeColor(status: string): "blue" | "green" | "amber" | "red" | "gray" {
...
export default async function DashboardPage() {
  ...
  if (!user) redirect("/login?message=Please sign in to access your dashboard.");
  ...
  let submissions: any[] = [];
  let submissionCounts = { draft: 0, submitted: 0, under_review: 0, approved: 0, rejected: 0, changes_requested: 0 };

  if (["agent", "builder", "lister", "super_admin"].includes(role)) {
    const { data } = await supabase
      .from("property_submissions")
      .select("id, title, city, state, status, intent, price, bedrooms, created_at, updated_at")
      .eq("owner_id", user.id)
  ...
  const totalListings = Object.values(submissionCounts).reduce((a, b) => a + b, 0);

  // ── Role-specific headings & actions ─────────────────────────────────────
  const roleConfig: Record<string, { title: string; sub: string; primaryAction: { label: string; href: string } }> = {
    user: {
    ...
    agent: {
      ...
      primaryAction: { label: "+ Add Listing", href: "/dashboard/add-property" },
    },
    builder: {
      title: `Project dashboard – ${firstName} 🏗️`,
      sub: "Monitor projects, track inventory and manage qualified leads.",
      primaryAction: { label: "+ Add Project", href: "/dashboard/add-property" },
    },
    lister: {
      title: `Hello, ${firstName} 🔑`,
      sub: "Submit your properties and track their review and approval status.",
      primaryAction: { label: "+ Submit Property", href: "/dashboard/add-property" },
    },
    super_admin: {
      title: `Admin Console ⚡`,
    ...
    <div className="p-[27px_clamp(18px,4vw,52px)_60px] max-w-[1300px] w-full mx-auto font-sans">
      ...
      <div className="flex items-center justify-between mb-[30px]">
        <div className="flex items-center gap-[10px]">
          <span className="text-[24px] leading-none">{ROLE_EMOJI[role] || "👤"}</span>
          <div>
            <p className="text-[10px] text-muted font-bold uppercase tracking-[0.07em] m-0">Dashboard</p>
            <p className="text-[13px] font-bold text-ink m-0">{ROLE_LABELS[role] || role}</p>
          </div>
        </div>
        <Link
      ...
      {role === "user" && (
        <>
          ...
          <section className="grid grid-cols-3 max-sm:grid-cols-1 gap-[14px] mb-[20px]">
            <StatCard label="Member since" value={new Date(profile?.created_at || Date.now()).toLocaleDateString("en-IN", { month: "short", year: "numeric" })} sub="Account active" accent="text-[#1c7cde]" />
            <StatCard label="Saved Properties" value="—" sub="Sign in on any device to sync" accent="text-[#d48a11]" />
            <StatCard label="Browse Properties" value="→" sub="Explore live listings now" accent="text-[#2d8a5f]" />
          </section>
          ...
          <div className="grid grid-cols-[1.4fr_0.8fr] max-lg:grid-cols-1 gap-[16px] mb-[16px]">
            ...
            <article className="bg-white border border-line rounded-[10px] p-[22px]">
              ...
              <div className="flex flex-col gap-[10px]">
                <Link href="/properties" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
                  <span className="text-[18px]">🔍</span> Browse Marketplace
                </Link>
                <Link href="/user-dashboard?view=saved" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
                  <span className="text-[18px]">❤️</span> Saved Properties
                </Link>
                <Link href="/user-dashboard?view=alerts" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
                  <span className="text-[18px]">🔔</span> Price Alerts
                </Link>
                <Link href="/user-dashboard?view=settings" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
      ...
      {["lister", "agent", "builder"].includes(role) && (
        <>
          ...
          <section className="grid grid-cols-4 max-md:grid-cols-2 max-sm:grid-cols-2 gap-[14px] mb-[20px]">
            <StatCard label="Total Submissions" value={totalListings || "0"} sub="All time" accent="text-[#1c7cde]" />
            <StatCard label="Approved" value={submissionCounts.approved || "0"} sub="Live on platform" accent="text-[#2d8a5f]" />
            <StatCard label="Under Review" value={submissionCounts.under_review + submissionCounts.submitted || "0"} sub="Awaiting admin check" accent="text-[#d48a11]" />
            <StatCard label="Action Needed" value={submissionCounts.changes_requested || "0"} sub="Changes requested" accent="text-[#c0392b]" />
          </section>
          ...
          <div className="grid grid-cols-[1.4fr_0.8fr] max-lg:grid-cols-1 gap-[16px] mb-[16px]">
            ...
            <article className="bg-white border border-line rounded-[10px] p-[22px]">
              <div className="flex justify-between items-center mb-[16px]">
                <h2 className="text-[15px] font-bold text-ink m-0">Recent Submissions</h2>
                <Link href="/dashboard/properties" className="text-[#a66d1c] text-[11px] font-bold hover:underline no-underline">
                  View all →
                </Link>
              </div>
              ...
              {submissions.length === 0 ? (
                ...
                <div className="grid gap-[10px]">
                  {submissions.map((sub) => (
                    <div key={sub.id} className="flex items-center gap-[14px] border border-[#eef0f2] rounded-[8px] p-[13px] hover:border-[#d4d8db] transition-colors">
                      <div className="w-[42px] h-[42px] rounded-[8px] bg-[#f3f5f7] flex items-center justify-center text-[20px] shrink-0">
                        {role === "builder" ? "🏗️" : "🏠"}
                      </div>
                      <div className="min-w-0 flex-1">
                        <h3 className="text-[12px] font-bold m-0 mb-[2px] text-ink truncate">{sub.title}</h3>
                        <p className="text-[11px] text-muted m-0">
                          {sub.city}, {sub.state}
                          {sub.price ? ` · ₹${Number(sub.price).toLocaleString("en-IN")}` : ""}
                        </p>
                      </div>
                      <Badge text={statusLabel(sub.status)} color={statusBadgeColor(sub.status)} />
                    </div>
                  ))}
                </div>
              )}
            ...
            <article className="bg-white border border-line rounded-[10px] p-[22px]">
              ...
              <div className="flex flex-col gap-[10px]">
                ...
                <Link href="/dashboard/properties" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
                  <span className="text-[18px]">📋</span> My Submissions
                </Link>
                <Link href="/dashboard/leads" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
                  <span className="text-[18px]">👥</span> Leads & Enquiries
                </Link>
                <Link href="/dashboard/billing" className="flex items-center gap-[10px] p-[12px] border border-line rounded-[8px] text-[13px] font-semibold text-ink hover:border-[#1c2b39] hover:bg-[#f8f9fa] transition-colors no-underline">
      ...
      {role === "super_admin" && (
        <div className="bg-[#1c2b39] rounded-[12px] p-[28px] text-white text-center">
          ...
          <h2 className="font-serif text-[24px] font-medium m-0 mb-[8px]">You have Super Admin access</h2>
          <p className="text-[13px] text-white/70 m-0 mb-[20px]">Access the full admin console to manage users, listings, settings and analytics.</p>
          <Link href="/super-admin" className="inline-flex items-center bg-[#e5ad43] text-[#1c2b39] rounded-[8px] px-[24px] h-[42px] text-[14px] font-bold hover:bg-[#f0be57] transition-colors no-underline">
            Open Admin Panel →
          </Link>
        </div>
      )}
