...
export default function PropertiesPage() {
  ...
  const [docs, setDocs] = useState<{ storage_path: string; document_type: string; file_name: string }[]>([]);
  ...
  const openDetail = async (sub: Submission) => {
    ...
    const { data: docData } = await supabase
      .from("property_submission_documents")
      .select("*")
      .eq("submission_id", sub.id);
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[75vh]">
              ...
              {docs.length > 0 && (
                <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl">
                  <h3 className="font-bold text-[15px] text-amber-900 mb-3 flex items-center gap-2">
                    📄 Proof of Ownership & Documents ({docs.length})
                  </h3>
                  <div className="grid grid-cols-2 max-sm:grid-cols-1 gap-3">
                    {docs.map((doc, idx) => {
                      const isImage = doc.storage_path.match(/\.(jpeg|jpg|gif|png)$/) != null || !doc.storage_path.endsWith(".pdf");
                      return (
                        <a
                          key={idx}
                          ...
                          className="flex items-center gap-3 p-3 bg-white border border-amber-200 rounded-xl hover:bg-amber-100 transition-colors"
                        >
                          <div className="flex-shrink-0 w-10 h-10 bg-amber-200 rounded flex items-center justify-center text-amber-800 font-bold text-lg">
                            {isImage ? "🖼️" : "📄"}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-[12px] font-bold text-amber-900 truncate">
                              {doc.file_name || "Document"}
                            </p>
                            <p className="text-[10px] text-amber-700 capitalize">
                              {doc.document_type || "Proof"}
                            </p>
                          </div>
                        </a>
                      );
                  ...
                  <p className="text-[11px] text-amber-700 mt-3 font-semibold">
                    * These documents are confidential and only visible to Super Admins.
                  </p>
                </div>
              )}
              ...
              <div className="border-t border-slate-200 pt-4 space-y-3">
                {showRequestChange ? (
                  <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl">
                    <h3 className="text-amber-800 font-bold text-[14px] mb-2">Request Changes from Owner</h3>
                    <p className="text-amber-700 text-[12px] mb-4">
                      Registered By: <strong>{selected.contact_name}</strong>
                    </p>
                    <label className="block mb-3">
                      <span className="block text-[12px] font-bold text-amber-900 mb-1">
                        Changes Description Box (Required)
                      </span>
                      <textarea
                        rows={4}
                        value={note}
                        ...
                        className="w-full border border-amber-300 rounded-xl p-2.5 text-[12px] bg-white outline-none focus:border-amber-500 resize-none"
                      />
                    </label>
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-[13px] py-2.5 rounded-xl transition-colors shadow-sm disabled:opacity-60"
                      >
                        Submit Request
                      </button>
                      <button
                        ...
                        disabled={working}
                        className="flex-1 bg-white text-slate-700 border border-slate-300 hover:bg-slate-50 font-bold text-[13px] py-2.5 rounded-xl transition-colors shadow-sm"
                      >
                        Cancel
                      </button>
                    </div>
